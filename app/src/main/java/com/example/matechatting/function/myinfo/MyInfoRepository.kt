...
import com.example.matechatting.function.forgetpassword.ForgetPasswordActivity.Companion.token
    ...
    private fun doOnSaveSuccess(token: String, callback: () -> Unit) {
        loginDao.getAllByToken(token)
            ...
            .subscribe({
                ...
                saveState(it.account, it.token, it.id,callback)
            }, {})
    }

    private fun saveState(account: String, token: String, id: Int,callback: () -> Unit) {
